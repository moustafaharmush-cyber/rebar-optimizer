...
# =========================
def optimize_cutting(lengths, iterations):
    best_solution = None
    min_waste = float("inf")
    min_bars = float("inf")
...
iterations = st.slider("Number of optimization iterations", min_value=100, max_value=3000, value=400, step=100)
...
if st.button("Run Optimization"):
    ...
    for d, lengths in data.items():
        solution = optimize_cutting(lengths, iterations)
        if not solution:
            continue
    ...
    # PDF
    pdf_file = generate_pdf(df, waste_df, purchase_df)
